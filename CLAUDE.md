...
- **Client:** React 19 + TypeScript, Vite 8, TailwindCSS 4, React Router, TanStack Query & Table, Recharts, React Hook Form + Zod 3, Axios
- **Server:** Express + TypeScript, Knex (query builder/migrations), better-sqlite3 (local) / PostgreSQL 16 (production via Neon), JWT auth, Pino logger
...
**Database:** SQLite via better-sqlite3 for local dev (default when `DATABASE_URL` is not set). PostgreSQL when `DATABASE_URL` is set — both `knexfile.ts` and `src/config/db.ts` auto-detect. DB file at `Source/server/data/sharadha_farm.db`. 8 tables: zones, workers, tasks, inventory_items, transactions, harvests, users, plus FK indexes. Migrations numbered `001`–`008` in `src/migrations/`. Dashboard SQL uses helper functions (`monthExpr`, `dateAgo12Months`) that emit the correct SQL for both SQLite and PostgreSQL.
...
- DataTable generic constraint is `T extends { id: string }` — do NOT use `Record<string, unknown>`
