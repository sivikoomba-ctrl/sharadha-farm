...
**NHB Subsidy feature:** Three tables track a 7-stage National Horticulture Board subsidy workflow. Routes at `/api/subsidy-applications` (JWT-protected). Singleton UI at `/subsidy` (`pages/SubsidyPage.tsx`) — empty state shows a create form; once an active application exists, renders header + 3-chip deadline bar (originals 7d / completion 18mo / claim 3mo, colored by urgency) + 7-step stage stepper + 4 tabs (Documents / Stages / Timeline / Notes). API client in `api/subsidy.ts`. Server-side load-bearing rule in `subsidy.service.ts`: cannot set `first_term_loan_release_date` without `goc_date`, and `advanceStage(5+)` requires `goc_date` — do NOT relax this, it's a real-world money rule. Deadline math is in TypeScript (`computeDeadlines`), not SQL, so it works on both SQLite and Postgres. Documents are auto-seeded (16 rows from `SUBSIDY_DOC_KEYS`) on application create and idempotently backfilled on `getById`. Kannada legal/document terms in `locales/kn.json` are prefixed `[KN-TODO]` pending consultant verification.
